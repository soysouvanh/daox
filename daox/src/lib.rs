...
"#,
        ...
        for (table_name, table_type) in tables {
            ...
            {
                continue; // Ignorer les tables au nom toxique
            }
            ...
            for col in columns {
                ...
                if !col_name
                    ...
                    .all(|c| c.is_ascii_alphanumeric() || c == '_')
                {
                    continue; // Ignorer les colonnes au nom toxique
                }
                ...
                if db_type == "sqlx::Postgres" {
                    ...
                    for (n, m) in &insert_cols {
                        ...
                        let fmt = match rust_type.as_str() {
                            ...
                            "Vec<u8>" => {
                                "format!(\"\\\\\\\\x{}\", v.iter().fold(String::new(), |mut acc, b| { std::fmt::Write::write_fmt(&mut acc, format_args!(\"{:02x}\", b)).ok(); acc }))"
                            }
                            "serde_json::Value" => {
