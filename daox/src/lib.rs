        ...
        match self {
            Dialect::MySql => {
                let updates: Vec<String> = update_cols
                    .iter()
                    .map(|c| format!("`{c}` = new_row.`{c}`"))
                    .collect();
                format!(" AS new_row ON DUPLICATE KEY UPDATE {}", updates.join(", "))
            }
            Dialect::Postgres => {
...
"#,
        ...
        #[cfg(feature = "mysql")]
        {
            ...
            for (table_name, table_type) in tables {
                ...
                for col in columns {
                    ...
                    let rust_type = if column_type == "tinyint(1)" || data_type == "boolean" {
                        "bool"
                    } else {
                        match data_type.as_str() {
                            ...
                            "float" | "double" | "decimal" | "numeric" => "f64",
    ...
    pub fn generate_dao(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for table in &tables {
            ...
            code.push_str(&format!(
                "    pub async fn list_paginated<'e, E: sqlx::Executor<'e, Database = {db}>>(executor: E, order_by: &str, page: u32, page_size: u32) -> sqlx::Result<Vec<Self>> {{\n\
                 \x20       const ALLOWED: &[&str] = &[{allowed}];\n\
                 \x20       for part in order_by.split(',') {{\n\
                 \x20           let col = part.trim().trim_end_matches(\" ASC\").trim_end_matches(\" DESC\").trim();\n\
                 \x20           if !ALLOWED.contains(&col) {{\n\
                 \x20               return Err(sqlx::Error::Protocol(format!(\"Invalid ORDER BY: {{}}\", col).into()));\n\
                 \x20           }}\n\
                 \x20       }}\n\
            ...
            if !table.config.is_view && !insert_cols.is_empty() {
                ...
                code.push_str(&format!(
                    "    pub async fn insert_batch<'e, E: sqlx::Executor<'e, Database = {db}> + Clone>(executor: E, items: &[Self]) -> sqlx::Result<u64> {{\n\
                     \x20       if items.is_empty() {{ return Ok(0); }}\n\
                     \x20       let chunk_size = 65000 / {col_count};\n\
                     \x20       let mut total_affected = 0;\n\
                     \x20       for chunk in items.chunks(chunk_size.max(1)) {{\n\
                     \x20           let mut qb: sqlx::QueryBuilder<{db}> = sqlx::QueryBuilder::new(\"INSERT INTO {tbl} ({cols}) \");\n\
                     \x20           qb.push_values(chunk, |mut b, item| {{\n",
                    db = db_type, tbl = sql_table, cols = ins_cols_str, col_count = col_count
                ));
                ...
                code.push_str(
                    "            });\n\
                     \x20           let result = qb.build().execute(executor.clone()).await?;\n\
                     \x20           total_affected += result.rows_affected();\n\
                     \x20       }\n\
                     \x20       Ok(total_affected)\n\
                     \x20   }\n\n",
                );
                ...
                if !pk_cols.is_empty() && !update_cols.is_empty() {
                    ...
                    code.push_str(&format!(
                        "    pub async fn upsert_batch<'e, E: sqlx::Executor<'e, Database = {db}> + Clone>(executor: E, items: &[Self]) -> sqlx::Result<u64> {{\n\
                         \x20       if items.is_empty() {{ return Ok(0); }}\n\
                         \x20       let chunk_size = 65000 / {col_count};\n\
                         \x20       let mut total_affected = 0;\n\
                         \x20       for chunk in items.chunks(chunk_size.max(1)) {{\n\
                ...
                if !pk_cols.is_empty() {
                    ...
                    if pk_cols.len() == 1 {
                        ...
                        code.push_str(&format!(
                            "    pub async fn delete_many_by_{sfx}<'e, E: sqlx::Executor<'e, Database = {db}> + Clone>(executor: E, ids: &[{idt}]) -> sqlx::Result<u64> {{\n\
                             \x20       if ids.is_empty() {{ return Ok(0); }}\n\
                             \x20       let mut total_affected = 0;\n\
                             \x20       for chunk in ids.chunks(500) {{\n\
                             \x20           let mut qb: sqlx::QueryBuilder<{db}> = sqlx::QueryBuilder::new(\"DELETE FROM {tbl} WHERE {pk} IN \");\n\
                             \x20           qb.push(\"(\");\n\
                             \x20           let mut sep = qb.separated(\", \");\n\
                             \x20           for id in chunk {{ sep.push_bind(id); }}\n\
                ...
                for (n, _) in &update_cols {
                    ...
                    code.push_str(&format!(
                        "        if let Some(val) = &patch.{f} {{\n\
                         \x20           has = true;\n\
                         \x20           sep.push(\"{cq} = \");\n\
                         \x20           sep.push_bind_unseparated(val);\n\
                         \x20       }}\n",
                        f = field,
                        cq = col_q,
                    ));
        ...
        let tmp_path = dest_path.with_extension("rs.tmp");
