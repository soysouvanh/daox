...
fn read_toml_file_limited(path: &std::path::Path) -> Result<String, Box<dyn std::error::Error>> {
    let meta = fs::metadata(path)?;
    if meta.len() > MAX_TOML_SIZE {
        return Err(format!(
            "TOML file {:?} exceeds size limit ({} > {} bytes)",
            path, meta.len(), MAX_TOML_SIZE
        ).into());
    }
    Ok(fs::read_to_string(path)?)
}

fn safe_remove_dir_all(path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
...
fn safe_write_if_changed<P: AsRef<std::path::Path>, C: AsRef<[u8]>>(
    path: P, 
    content: C
) -> Result<bool, String> {
    let p = path.as_ref();
...
"#,
            ...
            for entry in fs::read_dir(schema_path)? {
                ...
                if name != "overrides" && name != "databases.toml" && !table_names.contains(&name) {
                    if let Err(e) = safe_remove_dir_all(&entry.path()) {
                        eprintln!("Daox: warning: could not remove orphaned entry {:?}: {}", entry.path(), e);
                    }
                }
            }
        }
    ...
    pub fn generate_dao(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if tables.len() > MAX_TABLES {
            ...
            return Err(format!("Too many tables ({}) > limit ({})", tables.len(), MAX_TABLES).into());
        }
