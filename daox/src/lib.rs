        ...
        match self {
            ...
            Dialect::Postgres => {
                let conflict: Vec<String> =
                    pk_cols.iter().map(|c| self.quote_ident(c)).collect();
                let updates: Vec<String> = update_cols
                    .iter()
...
impl DaoGenerator {
    ...
    pub async fn introspect(&self, database_url: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let fallback_dialect = if is_postgres {
            ...
            if !is_mysql {
                println!("cargo:warning=Daox: Unknown dialect for url '{}'. Falling back to 'mysql'.", database_url);
            }
            "mysql"
        };
...
"#,
            ...
            write_if_changed(&databases_toml_path, databases_toml_content)?;
        }
        ...
        ).await.map_err(|_| "Table listing timeout after 10s")??;
        ...
        if schema_path.exists() {
            let table_names: std::collections::HashSet<_> = tables.iter().map(|(name, _)| name.clone()).collect();
            for entry in fs::read_dir(schema_path)? {
                let entry = entry?;
                let meta = fs::symlink_metadata(entry.path())?;
                if meta.file_type().is_symlink() {
                    return Err(format!("Symlink rejected in schema dir: {:?}", entry.path()).into());
                }
        ...
        for (table_name, table_type) in tables {
            ...
            spawn_handles.push(tokio::spawn(async move {
                ...
                let res: Result<(), Box<dyn std::error::Error + Send + Sync>> = async {
                    if table_name.is_empty()
                        ...
                        || table_name.chars().next().is_none_or(|c| c.is_ascii_digit())
                    {
                        println!(
            ...
            if let Some(desc) = existing_desc {
                writeln!(&mut toml_str, "description = {}", toml::to_string(&desc).unwrap()).unwrap();
            }
            if is_view {
                toml_str.push_str("is_view = true\n");
            }
            write_if_changed(&table_toml, toml_str)?;

            let columns = tokio::time::timeout(
                std::time::Duration::from_secs(10),
                sqlx::query(cols_query).bind(&table_name).fetch_all(&pool)
            ).await.map_err(|_| "Column fetch timeout")?.unwrap();
            ...
            for col in columns {
                ...
                if col_name.is_empty()
                    || !col_name
                        .chars()
                        .all(|c| c.is_ascii_alphanumeric() || c == '_')
                    || col_name.chars().next().is_none_or(|c| c.is_ascii_digit())
                {
                    println!(
                    ...
                    || data_type.to_lowercase() == "boolean"
                {
                    ...
                    if cn.contains("json") {
                        ...
                        | "numeric" => "f64",
                        "varchar" | "text" | "enum" | "char" | "character varying" => "String",
                        "timestamp"
                ...
                let format_regex = if col_name.starts_with("is_") || col_name.starts_with("has_") {
                    ...
                    let mut vals_str = column_type.replace("enum(", "").replace([')', '\''], "");
                    let vals_arr = vals_str
                        .split(',')
                        .map(|s| toml::to_string(&s).unwrap_or_else(|_| "\"\"".to_string()))
                        .collect::<Vec<_>>()
                        .join(", ");
                ...
                write!(&mut toml_content, 
                    ...
                    toml::to_string(&format_regex).unwrap_or_else(|_| "\"\"".to_string())
                ).unwrap();
                write!(&mut toml_content, "[is_primary_key]\nvalue = {}\n\n", is_primary_key).unwrap();
                write!(&mut toml_content, "[is_unique]\nvalue = {}\n\n", is_unique).unwrap();
                write!(&mut toml_content, "[is_index]\nvalue = {}\n\n", is_index).unwrap();
                write!(&mut toml_content, 
                    "[is_auto_increment]\nvalue = {}\n\n",
                    is_auto_increment
                ).unwrap();
                write!(&mut toml_content, "[has_default]\nvalue = {}\n\n", has_default_val).unwrap();
                write!(&mut toml_content, "[is_generated]\nvalue = {}\n\n", is_generated).unwrap();
                toml_content.push_str("[business_rules]\n\n");

                let col_file = table_dir.join(format!("{}.toml", col_name));
                write_if_changed(col_file, toml_content)?;
            }
            ...
            if !grouped.is_empty() {
                ...
                for (name, (is_unique, cols)) in grouped {
                    s.push_str("[[indexes]]\n");
                    writeln!(&mut s, "name = \"{}\"", name).unwrap();
                    writeln!(&mut s, "is_unique = {}", is_unique).unwrap();
                    let cols_str = cols
                        .iter()
                        .map(|c| format!("\"{}\"", c))
                        .collect::<Vec<_>>()
        ...
        for handle in spawn_handles {
            handle.await.map_err(|e| format!("Spawn error: {}", e).into())
    ...
    pub fn parse_schema(&self) -> Result<Vec<TableMetadata>, Box<dyn std::error::Error>> {
        ...
        let parse_table_dir = |dir_path: &Path| -> TableDirParseResult {
            ...
            for col_entry in fs::read_dir(dir_path)? {
                ...
                {
                    let file_stem = col_path.file_stem().and_then(|s| s.to_str()).ok_or("Invalid path")?.to_string();
                    if file_stem != "_table" && file_stem != "_indexes" && (!file_stem.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') || file_stem.chars().next().is_none_or(|c| c.is_ascii_digit())) {
                        continue;
                    }
                    ...
                    const ALLOWED_TYPES: &[&str] = &[
                        ...
                        "f32", "f64", "String", "Vec<u8>", "serde_json::Value",
                        "chrono::NaiveDate", "chrono::DateTime<chrono::Utc>",
                        "chrono::NaiveDateTime", "json", "jsonb",
                    ];
                    let resolved = col_meta.rust_type_resolved();
                    if !ALLOWED_TYPES.contains(&resolved.as_str()) {
                        return Err(format!("Column '{}': forbidden type '{}'", file_stem, resolved).into());
                    }
        ...
        for entry in fs::read_dir(schema_path)? {
            ...
            if path.is_dir() {
                ...
                if has_toml {
                    let table_name = entry.file_name().into_string().map_err(|_| "Invalid utf8 table name")?;
                    if table_name.is_empty() || !table_name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') || table_name.chars().next().is_none_or(|c| c.is_ascii_digit()) {
                        eprintln!("Daox: table directory '{}' rejected (invalid identifier)", table_name);
                        continue;
                    }
                    dirs_to_parse.push((path, None, table_name));
                } else {
                    let db_name = entry.file_name().into_string().map_err(|_| "Invalid utf8 db name")?;
                    if let Ok(subs) = fs::read_dir(&path) {
                        for sub in subs.flatten() {
                            if sub.path().is_dir() {
                                let table_name = sub.file_name().into_string().map_err(|_| "Invalid utf8 table name")?;
                                if table_name.is_empty() || !table_name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') || table_name.chars().next().is_none_or(|c| c.is_ascii_digit()) {
                                    eprintln!("Daox: table directory '{}/{}' rejected (invalid identifier)", db_name, table_name);
                                    continue;
                                }
        ...
        use rayon::prelude::*;
        let parsed_results: Result<Vec<TableMetadata>, String> = dirs_to_parse.into_par_iter()
            .map(|(path, db_name_opt, table_name)| -> Result<TableMetadata, String> {
                let (mut config, columns, indexes) = parse_table_dir(&path).map_err(|e| e.to_string())?;
        ...
        let overrides_path = match std::env::var("DAOX_OVERRIDES_DIR") {
            Ok(dir) => {
                ...
                let canonical_base = fs::canonicalize(schema_path.parent().unwrap())?;
                if !canonical.starts_with(&canonical_base) {
                    return Err(format!("Path traversal detected: {:?} is outside {:?}", canonical, canonical_base).into());
                }
                canonical
            },
            Err(_) => {
                let default = schema_path.parent().unwrap().join("overrides");
                if default.exists() {
        ...
        if overrides_path.is_dir() {
            for entry in fs::read_dir(overrides_path)? {
                ...
                let path = entry.path();
                if path.is_dir() {
                    let table_name = entry.file_name().into_string().map_err(|_| "Invalid UTF-8 in override filename")?;
                    if let Some(table) = tables.iter_mut().find(|t| t.name == table_name) {
                        for col_entry in fs::read_dir(&path)? {
                            ...
                            {
                                let file_stem =
                                    col_path.file_stem().and_then(|s| s.to_str()).ok_or("Invalid utf8 override path")?.to_string();
                                let content = fs::read_to_string(&col_path)?;
                                if file_stem == "_table" {
                                ...
                                if let Some(col_meta) = table.columns.get_mut(&file_stem) {
                                    ...
                                    if let Some(v) = ov.rust_type {
                                        ...
                                        if !ALLOWED_TYPES.contains(&resolved.as_str()) {
                                            return Err(format!("Column override '{}': forbidden type '{}'", file_stem, resolved).into());
                                        }
                                    }
    ...
    pub fn generate_dao(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let results: Result<Vec<_>, String> = tables.par_iter().map(|table| {
            ...
            // validate method
            write!(&mut code, "    #[allow(unused_comparisons)]\n    pub fn validate(&self) -> Result<(), Vec<String>> {{\n        let mut errors = Vec::new();\n").unwrap();
            for (col_name, col_meta) in &cols {
                let field = Self::escape_rust_keyword(col_name);
                            ...
                            write!(&mut code, 
                                "        #[cfg(feature = \"validation\")]\n\
                                         if let Some(v) = {val_ref} {{\n\
                                             ...
                                             let re = RE.get_or_init(|| regex::Regex::new({re}).expect(\"Invalid regex in TOML\"));\n\
                                             if !re.is_match(v) {{\n\
            ...
            if dialect == Dialect::Postgres {
                write!(&mut code, 
                    "    /// Returns an approximate total number of rows in the table using database statistics (O(1)).\n\
                         ...
                         pub async fn approximate_count<'e, E: sqlx::Executor<'e, Database = {db}>>(executor: E) -> sqlx::Result<u64> {{\n\
                             let query = r#\"SELECT reltuples::bigint FROM pg_class WHERE relname = '{tbl}'\"#;\n\
                ...
                write!(&mut code, 
                    "    /// Returns an approximate total number of rows in the table using database statistics (O(1)).\n\
                         ...
                         pub async fn approximate_count<'e, E: sqlx::Executor<'e, Database = {db}>>(executor: E) -> sqlx::Result<u64> {{\n\
                             let query = r#\"SELECT table_rows FROM information_schema.tables WHERE table_name = '{tbl}' AND table_schema = DATABASE()\"#;\n\
                             let count: Option<(i64,)> = sqlx::query_as(query).fetch_optional(executor).await?;\n\
                             Ok(count.map(|(c,)| c.max(0) as u64).unwrap_or(0))\n\
                         }}\n\n",
                    db = db_type,
            ...
            let default_order_sql = if pk_cols.is_empty() {
                let fallback = table.indexes.first().and_then(|idx| idx.columns.first().map(|s| s.as_str()))
                    .or_else(|| cols.iter().find(|(n, _)| **n == "created_at" || **n == "id").map(|(n, _)| n.as_str()))
                    .unwrap_or_else(|| cols.first().unwrap().0.as_str());
            ...
            if !table.config.is_view && !insert_cols.is_empty() {
                ...
                write!(&mut code,
                    "    pub async fn insert_validated<'e, E: sqlx::Executor<'e, Database = {db}>>(&self, executor: E) -> Result<u64, Box<dyn std::error::Error + Send + Sync>> {{\n\
                ...
                if !table.config.is_view && !pk_cols.is_empty() && !update_cols.is_empty() {
                    ...
                    write!(&mut code,
                        "    pub async fn upsert_validated<'e, E: sqlx::Executor<'e, Database = {db}>>(&self, executor: E) -> Result<u64, Box<dyn std::error::Error + Send + Sync>> {{\n\
                                 self.validate().map_err(|e| e.join(\", \"))?;\n\
                ...
                if !table.config.is_view && !pk_cols.is_empty() && !update_cols.is_empty() {
                    ...
                    write!(&mut code,
                        "    pub async fn update_validated_by_{sfx}<'e, E: sqlx::Executor<'e, Database = {db}>>(&self, executor: E) -> Result<u64, Box<dyn std::error::Error + Send + Sync>> {{\n\
                                 self.validate().map_err(|e| e.join(\", \"))?;\n\
                                 self.update_by_{sfx}(executor).await.map_err(|e| e.into())\n\
                             }}\n\n",
            ...
            if !table.config.is_view && !pk_cols.is_empty() && !update_cols.is_empty() {
                ...
                if update_cols.len() > 15 {
                    return Err(format!(
                        "Table '{}': {} update columns exceeds the 15-column limit for update_partial_by_id",
                        table.name, update_cols.len()
                    ));
                }
                ...
                for (i, (n, _)) in update_cols.iter().enumerate() {
                    let field = Self::escape_rust_keyword(n);
                    write!(&mut code, 
                        "        if patch.{f}.is_some() {{\n\
                                     mask |= 1 << {bit_idx};\n\
                ...
                write!(&mut code, "        if !has {{ return Ok(0); }}\n\n").unwrap();

                writeln!(&mut code, "        let query_str = match mask {{").unwrap();
                let num_combinations = 1u64 << update_cols.len().min(63);
                for m in 1..num_combinations {
                    let mut q = String::with_capacity(256 + update_cols.len() * 32);
                    q.push_str("UPDATE ");
                    q.push_str(&sql_table_q);
                    q.push_str(" SET ");
                    let mut first = true;
                    let mut param_idx = 1;
                    for (i, (n, _)) in update_cols.iter().enumerate() {
                        if (m & (1 << i)) != 0 {
                            if !first { q.push_str(", "); }
                            q.push_str(&dialect.quote_ident(n));
                            q.push_str(" = ");
                            if dialect == Dialect::Postgres {
                                use std::fmt::Write;
                    ...
                    let mut pk_where_gen: String = String::new();
                    for (i, (n, _)) in pk_cols.iter().enumerate() {
                        let col_q = dialect.quote_ident(n);
                        let ph = if dialect == Dialect::Postgres {
                            let mut buf = String::new();
                            use std::fmt::Write;
                            write!(&mut buf, "${}", param_idx).unwrap();
                            param_idx += 1;
                            buf
                        } else {
                            "?".to_string()
                        ...
                        use std::fmt::Write;
                        write!(&mut pk_where_gen, "{}{} = {}", conj, col_q, ph).unwrap();
                    }
                    q.push_str(&pk_where_gen);
                    writeln!(&mut code, "            {} => r#\"{}\"#,", m, q).unwrap();
                }
                write!(&mut code, "            _ => return Err(sqlx::Error::Protocol(\"invalid patch mask\".into())),\n        }};\n\n").unwrap();

                writeln!(&mut code, "        let mut query = sqlx::query::<{db}>(query_str);", db = db_type).unwrap();
                for (n, _) in &update_cols {
                    let field = Self::escape_rust_keyword(n);
                    writeln!(&mut code, 
                        "        if let Some(val) = &patch.{f} {{ query = query.bind(val); }}",
                        f = field
                    ).unwrap();
                }
                for (n, _) in &pk_cols {
