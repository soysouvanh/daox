...
"#,
        ...
        let (tables_query, cols_query) = if is_postgres {
            ...
            (
                ...
                SELECT 
                    ...
                    CASE WHEN hidden >= 2 THEN 1 ELSE 0 END AS IS_GENERATED
        ...
        for (table_name, table_type) in tables {
            ...
            {
                panic!(
                    "Security Error: Table name '{}' contains invalid characters. Only alphanumeric and underscores are allowed to prevent schema poisoning.",
                    table_name
                );
            }
            ...
            for col in columns {
                ...
                {
                    panic!(
                        "Security Error: Column name '{}' contains invalid characters. Only alphanumeric and underscores are allowed to prevent schema poisoning.",
                        col_name
                    );
                }
                    ...
                    || data_type.to_lowercase() == "boolean"
                {
                    ...
                    match d_type.as_str() {
                        ...
                        _ => {
                            if col_name.to_lowercase().contains("json") {
                                "json"
                            } else {
                                "String"
                            }
                ...
                toml_content.push_str(&format!("[has_default]\nvalue = {}\n\n", has_default_val));
                toml_content.push_str(&format!("[is_generated]\nvalue = {}\n\n", is_generated));
        ...
        for table in &tables {
            ...
            code.push_str(&format!(
                "    pub fn stream_all<'e, E: sqlx::Executor<'e, Database = {db}> + 'e>(executor: E) -> impl futures::Stream<Item = sqlx::Result<Self>> + 'e {{\n\
                         let query = \"SELECT {cols} FROM {tbl} {order}\";\n\
            ...
            // list_paginated
            if pk_cols.len() != 1 {
                let allowed_cols = cols
                    .iter()
                    .map(|(n, _)| format!("\"{}\"", Self::escape_rust_keyword(n)))
                    .collect::<Vec<_>>()
                    .join(", ");
                code.push_str(&format!(
                    "    pub async fn list_paginated<'e, E: sqlx::Executor<'e, Database = {db}>>(executor: E, order_by: &str, page: u32, page_size: u32) -> sqlx::Result<Vec<Self>> {{\n\
                             ...
                             let mut qb: sqlx::QueryBuilder<{db}> = sqlx::QueryBuilder::new(\"SELECT {cols} FROM {tbl} ORDER BY \");\n\
                             qb.push(valid_order);\n\
                             qb.push(\" LIMIT \");\n\
            ...
            if !pk_cols.is_empty() {
                ...
                code.push_str(&format!(
                    "    pub async fn get_by_{sfx}<'e, E: sqlx::Executor<'e, Database = {db}>>(executor: E, {args}) -> sqlx::Result<Option<Self>> {{\n\
                             let query = \"SELECT {cols} FROM {tbl} WHERE {wh}\";\n\
                ...
                code.push_str(&format!(
                    "    pub async fn exists_by_{sfx}<'e, E: sqlx::Executor<'e, Database = {db}>>(executor: E, {args}) -> sqlx::Result<bool> {{\n\
                             let query = \"SELECT 1 FROM {tbl} WHERE {wh} LIMIT 1\";\n\
                ...
                if pk_cols.len() == 1 {
                    ...
                    code.push_str(&format!(
                        "    pub async fn list_by_cursor<'e, E: sqlx::Executor<'e, Database = {db}>>(executor: E, last_id: {cat}, limit: u32) -> sqlx::Result<Vec<Self>> {{\n\
                                 let query = \"SELECT {cols} FROM {tbl} WHERE {pk} > {p1} ORDER BY {pk} ASC LIMIT {p2}\";\n\
            ...
            if !table.config.is_view && !insert_cols.is_empty() {
                ...
                if db_type == "sqlx::Postgres" {
                    code.push_str(&format!(
                        "    /// Inserts a batch of records using Postgres COPY (ultra-fast). \n\
                             ...
                             pub async fn insert_batch<'e>(executor: &mut sqlx::Transaction<'e, {db}>, items: &[Self]) -> sqlx::Result<u64> {{\n\
                                 ...
                                 let mut copy_in = executor.copy_in_raw(\"COPY {tbl} ({cols}) FROM STDIN WITH (FORMAT csv)\").await?;\n\
                    ...
                    for (n, m) in &insert_cols {
                        ...
                        let fmt = match rust_type.as_str() {
                            "String" => {
                                "format!(\"\\\"{}\\\"\", v.replace(\"\\\"\", \"\\\"\\\"\"))"
                            }
                            "chrono::DateTime<chrono::Utc>"
                            | "chrono::NaiveDate"
                            | "chrono::NaiveDateTime" => "format!(\"\\\"{}\\\"\", v)",
                            "Vec<u8>" => {
                                "{ let mut s = String::with_capacity(3 + v.len() * 2); s.push_str(\"\\\\\\\\x\"); for b in v { use std::fmt::Write; write!(&mut s, \"{:02x}\", b).ok(); } s }"
                            }
                            "serde_json::Value" => {
                            ...
                            _ => "v.to_string()", // Numeric
                        };
                        let val_expr = if is_opt {
                            format!(
                                "if let Some(v) = &item.{f} {{ {fmt} }} else {{ String::new() }}",
                                f = field,
                                fmt = fmt
                            )
                        } else {
                            format!("{{ let v = &item.{f}; {fmt} }}", f = field, fmt = fmt)
                        };
                        if i > 0 {
                        ...
                        "    /// Inserts a batch of records. \n\
                             ...
                             pub async fn insert_batch<'e>(executor: &mut sqlx::Transaction<'e, {db}>, items: &[Self]) -> sqlx::Result<u64> {{\n\
                                 ...
                                 for chunk in items.chunks(chunk_size.max(1)) {{\n\
                                     let mut qb: sqlx::QueryBuilder<{db}> = sqlx::QueryBuilder::new(\"INSERT INTO {tbl} ({cols}) \");\n\
                ...
                if !table.config.is_view && !pk_cols.is_empty() && !update_cols.is_empty() {
                    ...
                    code.push_str(&format!(
                        "    pub async fn update_by_{sfx}<'e, E: sqlx::Executor<'e, Database = {db}>>(&self, executor: E) -> sqlx::Result<u64> {{\n\
                                 let query_str = \"UPDATE {tbl} SET {set} WHERE {wh}\";\n\
                ...
                if !table.config.is_view && !pk_cols.is_empty() {
                    ...
                    code.push_str(&format!(
                        "    pub async fn delete_by_{sfx}<'e, E: sqlx::Executor<'e, Database = {db}>>(executor: E, {args}) -> sqlx::Result<u64> {{\n\
                                 let query = \"DELETE FROM {tbl} WHERE {wh}\";\n\
                                 let result = sqlx::query::<{db}>(query){binds}.execute(executor).await?;\n\
                                 Ok(result.rows_affected())\n\
                             }}\n\n",
                        ...
                        code.push_str(&format!(
                            "    pub async fn delete_many_by_{sfx}<'e>(executor: &mut sqlx::Transaction<'e, {db}>, ids: &[{idt}]) -> sqlx::Result<u64> {{\n\
                                     ...
                                     for chunk in ids.chunks({max_params}) {{\n\
                                         let mut qb: sqlx::QueryBuilder<{db}> = sqlx::QueryBuilder::new(\"DELETE FROM {tbl} WHERE {pk} IN \");\n\
                                         qb.push(\"(\");\n\
                ...
                if index.is_unique {
                    ...
                    code.push_str(&format!(
                        "    pub async fn list_by_{sfx}<'e, E: sqlx::Executor<'e, Database = {db}>>(executor: E, {args}, limit: i64) -> sqlx::Result<Vec<Self>> {{\n\
                                 let query = \"SELECT {cols} FROM {tbl} WHERE {wh} {order} LIMIT {lph}\";\n\
                    ...
                    code.push_str(&format!(
                        "    pub fn stream_by_{sfx}<'e, E: sqlx::Executor<'e, Database = {db}> + 'e>(executor: E, {args}) -> impl futures::Stream<Item = sqlx::Result<Self>> + 'e {{\n\
                                 let query = \"SELECT {cols} FROM {tbl} WHERE {wh} {order}\";\n\
                                 sqlx::query_as::<_, Self>(query){binds}.fetch(executor)\n\
                             }}\n\n",
            ...
            write_if_changed(&table_file, &code)?;
