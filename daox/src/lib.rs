...
fn parse_mysql_enum(input: &str) -> Result<Vec<String>, String> {
...
fn ensure_safe_dir(base: &std::path::Path, p: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
...
"#,
        ...
        for (table_name, table_type) in tables {
            ...
            if let Ok(content) = fs::read_to_string(&table_toml_path)
            ...
            write_if_changed(&table_toml_path, toml_str)?;
            ...
            for col in columns {
                ...
                let format_regex = if col_name.starts_with("is_") || col_name.starts_with("has_") {
                    ...
                    let parsed_vals = parse_mysql_enum(vals_str)
            ...
            if !grouped.is_empty() {
                ...
                write_if_changed(&idx_file, content)?;
