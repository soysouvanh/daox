...
"#,
        ...
        for (table_name, table_type) in tables {
            ...
            if let Some(desc) = existing_desc {
                writeln!(&mut toml_str, "description = {:?}", desc).unwrap();
            }
            if is_view {
            ...
            for col in columns {
                ...
                let format_regex = if col_name.starts_with("is_") || col_name.starts_with("has_") {
                    ...
                    for c in vals_str.chars() {
                        ...
                        else if c == ',' && !in_quote { parsed_vals.push(current.trim().to_string()); current.clear(); }
                        else { current.push(c); }
                    }
                    if !current.trim().is_empty() { parsed_vals.push(current.trim().to_string()); }
                    
                    let parsed_vals: Vec<String> = parsed_vals.into_iter().map(|s| {
                        if s.starts_with('\'') && s.ends_with('\'') && s.len() >= 2 {
                            s[1..s.len()-1].to_string()
                        } else { s }
                    }).collect();

                    let vals_arr = parsed_vals.iter()
                        .map(|s| format!("{:?}", s))
                        .collect::<Vec<_>>()
                        .join(", ");
    ...
    fn resolve_dialect(&self, db_name: &str) -> Dialect {
        ...
        println!("cargo:warning=Daox: Unknown dialect alias for DB '{}' in databases.toml. Falling back to 'mysql'.", db_name);
        Dialect::MySql
    }
            ...
            if !table.config.is_view && !pk_cols.is_empty() && !update_cols.is_empty() {
                ...
                for (n, _) in update_cols.iter() {
                    ...
                    if dialect == Dialect::Postgres {
                        write!(&mut code, 
                            "        if patch.{f}.is_some() {{\n\
                                         set_clauses.push(format!(\"{col} = {ph}\", _param_idx));\n\
                                         _param_idx += 1;\n\
                                     }}\n",
                            f = field, col = col_q, ph = ph
                        ).unwrap();
                    } else {
                        write!(&mut code, 
                            "        if patch.{f}.is_some() {{\n\
                                         set_clauses.push(\"{col} = {ph}\".to_string());\n\
                                         _param_idx += 1;\n\
                                     }}\n",
                            f = field, col = col_q, ph = ph
                        ).unwrap();
                ...
                write!(&mut code, 
                    "        if set_clauses.is_empty() {{ return Ok(0); }}\n\
                             let mut query_str = format!(\"UPDATE {tbl} SET {{}}\", set_clauses.join(\", \"));\n",
                    tbl = sql_table_q.replace('"', "\\\"")
                ).unwrap();
                ...
                for (i, (n, _)) in pk_cols.iter().enumerate() {
                    ...
                    if dialect == Dialect::Postgres {
                        write!(&mut code, 
                            "        query_str.push_str(&format!(\"{conj}{col} = ${{}}\", _param_idx));\n\
                                     _param_idx += 1;\n",
                            conj = conj, col = col_q
                        ).unwrap();
                    } else {
                        write!(&mut code, 
                            "        query_str.push_str(\"{conj}{col} = ?\");\n\
                                     _param_idx += 1;\n",
                            conj = conj, col = col_q
                        ).unwrap();
                    }
                ...
                writeln!(&mut code, 
                    "        static CACHE: std::sync::OnceLock<std::sync::RwLock<std::collections::HashMap<String, &'static str>>> = std::sync::OnceLock::new();\n\
                             ...
                             let mut query = sqlx::query::<{db}>(safe_query_str);", 
                    db = db_type
                ).unwrap();
                for (n, _) in &update_cols {
                    let field = Self::escape_rust_keyword(n);
...
mod toml_injection_tests {
    ...
    #[test]
    fn test_format_regex_with_single_quotes() {
        let regex = "^it's$";
...
mod path_traversal_tests {
    ...
    fn test_reject_path_traversal_overrides() {
        unsafe { std::env::set_var("DAOX_OVERRIDES_DIR", "../../etc"); }
        assert_eq!(std::env::var("DAOX_OVERRIDES_DIR").unwrap(), "../../etc");
        unsafe { std::env::remove_var("DAOX_OVERRIDES_DIR"); }
    }
}
