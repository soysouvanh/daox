            ...
            spawn_handles.push(tokio::spawn(async move {
                let res: Result<(), Box<dyn std::error::Error + Send + Sync>> = async {
                    if table_name.is_empty()
                        ...
                        || table_name.chars().next().map_or(true, |c| c.is_ascii_digit())
                    {
                        println!(
            ...
            for col in columns {
                ...
                if col_name.is_empty()
                    || !col_name
                        .chars()
                        .all(|c| c.is_ascii_alphanumeric() || c == '_')
                    || col_name.chars().next().map_or(true, |c| c.is_ascii_digit())
                {
                    println!(
        ...
        let parse_table_dir = |dir_path: &Path| -> TableDirParseResult {
            ...
            for col_entry in fs::read_dir(dir_path)? {
                    ...
                    && col_path.extension().and_then(|s| s.to_str()) == Some("toml")
                {
                    ...
                    if file_stem != "_table" && file_stem != "_indexes" && (!file_stem.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') || file_stem.chars().next().map_or(true, |c| c.is_ascii_digit())) {
                        continue;
                    }
        ...
        for entry in fs::read_dir(schema_path)? {
            ...
            if path.is_dir() {
                ...
                if has_toml {
                    ...
                    if table_name.is_empty() || !table_name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') || table_name.chars().next().unwrap().is_ascii_digit() {
    ...
    pub fn generate_dao(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let results: Result<Vec<_>, String> = tables.par_iter().map(|table| {
            ...
            for (col_name, col_meta) in &cols {
                        ...
                        if let Some(prop) = &col_meta.format {
                            ...
                            if !re_str.is_empty() {
                                if let Err(err) = regex::Regex::new(&re_str) {
                                    return Err(format!("Column '{}': invalid regex format '{}': {}", field, re_str, err).into());
                                }
                ...
                } else if ty != "bool" && ty != "serde_json::Value" && !ty.starts_with("chrono::") {
                ...
                if db_type == "sqlx::Postgres" {
                    ...
                    for (n, m) in &insert_cols {
                        ...
                        match rust_type.as_str() {
                            "String" => {
                                if is_opt {
                                    write!(&mut code, "                                         s += item.{f}.as_ref().map_or(1, |v| v.len() + 2);\n", f = field).unwrap();
                                } else {
                                    write!(&mut code, "                                         s += item.{f}.len() + 2;\n", f = field).unwrap();
                                }
                            }
                            "Vec<u8>" => {
                                if is_opt {
                                    write!(&mut code, "                                         s += item.{f}.as_ref().map_or(1, |v| v.len() * 2 + 4);\n", f = field).unwrap();
                                } else {
                                    write!(&mut code, "                                         s += item.{f}.len() * 2 + 4;\n", f = field).unwrap();
                                }
                            }
                            "serde_json::Value" => {
                                if is_opt {
                                    write!(&mut code, "                                         s += item.{f}.as_ref().map_or(1, |v| v.to_string().len() + 2);\n", f = field).unwrap();
                                } else {
                                    write!(&mut code, "                                         s += item.{f}.to_string().len() + 2;\n", f = field).unwrap();
                                }
                            }
                            _ => {
                                write!(&mut code, "                                         s += 32;\n").unwrap();
                    ...
                    for (i, (n, m)) in insert_cols.iter().enumerate() {
                        ...
                        let write_stmt = match rust_type.as_str() {
                            "String" => {
                                r#"payload.push('"'); for c in v.chars() { if c == '"' { payload.push_str("\"\""); } else if c == '\\' { payload.push_str("\\\\"); } else { payload.push(c); } } payload.push('"');"#
                            }
                            ...
                            "serde_json::Value" => {
                                r#"let json_str = v.to_string(); payload.push('"'); for c in json_str.chars() { if c == '"' { payload.push_str("\"\""); } else if c == '\\' { payload.push_str("\\\\"); } else { payload.push(c); } } payload.push('"');"#
                            }
                            "bool" => {
            ...
            if !table.config.is_view && !pk_cols.is_empty() && !update_cols.is_empty() {
                ...
                if update_cols.len() > 63 {
                    return Err(format!(
                        "Table '{}': {} update columns exceeds the 63-column limit for update_partial_by_id",
                ...
                write!(&mut code, "        let query_str = match mask {{\n").unwrap();
