            ...
            for col in columns {
                ...
                let format_regex = if col_name.starts_with("is_") || col_name.starts_with("has_") {
                    ...
                    for c in vals_str.chars() {
                        if escape { current.push(c); escape = false; prev_was_quote = false; }
                        else if c == '\\' { escape = true; prev_was_quote = false; }
                        else if c == '\'' {
                            if in_quote && prev_was_quote {
                                // '' → literal single quote (MySQL escaping)
                                current.push('\'');
                                prev_was_quote = false;
                            } else if in_quote {
                                // Potentially closing quote, but might be ''
                                prev_was_quote = true;
                            } else {
                                in_quote = true;
                                prev_was_quote = false;
                            ...
                            if prev_was_quote {
                                // Previous quote was actually closing the string
                                in_quote = false;
                                prev_was_quote = false;
                            }
                            current.push(c);
                    ...
                    // End of string - no need to update in_quote since it's no longer used
    ...
    pub fn parse_schema(&self) -> Result<Vec<TableMetadata>, Box<dyn std::error::Error>> {
            ...
            if path.is_dir() {
                ...
                if has_toml {
                    ...
                    if db_name.is_empty()
                        ...
                        || db_name.chars().next().is_none_or(|c| c.is_ascii_digit())
                    {
                        eprintln!("Daox: database directory '{}' rejected (invalid identifier)", db_name);
                        continue;
                    }
                    if let Ok(subs) = fs::read_dir(&path) {
                        for sub in subs.flatten() {
        ...
        let overrides_path = match std::env::var("DAOX_OVERRIDES_DIR") {
            Ok(dir) => {
                ...
                let canonical_base = fs::canonicalize(schema_path.parent().ok_or("schema_dir has no parent directory")?)?;
            ...
            Err(_) => {
                let default = schema_path.parent().ok_or("schema_dir has no parent directory")?.join("overrides");
                if default.exists() {
        ...
        if overrides_path.is_dir() {
            for entry in fs::read_dir(overrides_path)? {
                ...
                if dir_meta.file_type().is_symlink() {
                    return Err(format!(
                        "Symlink rejected for override table dir: {:?}", path
                    ).into());
                }
                if path.is_dir() {
                    ...
                    if let Some(table) = tables.iter_mut().find(|t| t.name == table_name) {
                        for col_entry in fs::read_dir(&path)? {
                            ...
                            {
                                let col_meta_fs = fs::symlink_metadata(&col_path)
                                    .map_err(|e| format!("Cannot stat override file {:?}: {}", col_path, e))?;
                                if col_meta_fs.file_type().is_symlink() {
                                    return Err(format!("Symlink rejected in overrides: {:?}", col_path).into());
                                }
                                let file_stem =
