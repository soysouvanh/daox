        ...
        let parse_table_dir = |dir_path: &Path| -> TableDirParseResult {
            ...
            for col_entry in fs::read_dir(dir_path)? {
                    ...
                    && col_path.extension().and_then(|s| s.to_str()) == Some("toml")
                {
                    ...
                    if file_stem != "_table" && file_stem != "_indexes" && !file_stem.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
                        continue;
                    }
            ...
            if path.is_dir() {
                ...
                if has_toml {
                    ...
                    if !table_name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
                        println!(
                            "cargo:warning=Daox: table directory '{}' skipped (invalid identifier characters)",
                            table_name
                        );
                        continue;
                    }
    ...
    pub fn generate_dao(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let results: Result<Vec<_>, String> = tables.par_iter().map(|table| {
            ...
            for (col_name, col_meta) in &cols {
                ...
                } else if ty != "bool" && ty != "Vec<u8>" && ty != "serde_json::Value" && !ty.starts_with("chrono::") {
            ...
            if dialect == Dialect::Postgres {
                ...
                if table.config.is_view {
                    ...
                    write!(&mut code, 
                        ...
                        db = db_type, tbl = sql_table
                    ).unwrap();
                }
            }

            let default_order_col = pk_cols
                .first()
                .map(|(n, _)| n.as_str())
                .or_else(|| {
                    table.indexes.first().and_then(|idx| idx.columns.first().map(|s| s.as_str()))
                })
                .or_else(|| {
                    cols.iter()
            ...
            let default_order_sql =
                format!("ORDER BY {} ASC", dialect.quote_ident(default_order_col));
            ...
            if pk_cols.len() != 1 {
                write!(&mut code, 
                    "    #[deprecated(note = \"Use list_by_cursor for large datasets\")]\n\
                         pub async fn list_paginated<'e, E: sqlx::Executor<'e, Database = {db}>>(executor: E, order_by: &[{enum_name}], page: u32, page_size: u32) -> sqlx::Result<Vec<Self>> {{\n\
                             if order_by.is_empty() {{\n\
                                 return Err(sqlx::Error::Protocol(\"ORDER BY cannot be empty\".into()));\n\
                             }}\n\
                             let page_size = page_size.clamp(1, 10000);\n\
                             let offset = page.saturating_sub(1) * page_size;\n\
                             let mut qb: sqlx::QueryBuilder<{db}> = sqlx::QueryBuilder::new(r#\"SELECT {cols} FROM {tbl}\"#);\n\
                             qb.push(\" ORDER BY \");\n\
            ...
            if !table.config.is_view && !insert_cols.is_empty() {
                ...
                if db_type == "sqlx::Postgres" {
                    ...
                    write!(&mut code, 
                        ...
                        db = db_type, tbl = sql_table, cols = ins_cols_str, col_count = col_count, max_params = max_params
                    ).unwrap();
                ...
                if !table.config.is_view && !pk_cols.is_empty() {
                    ...
                    if pk_cols.len() == 1 {
                        ...
                        write!(&mut code, 
                            ...
                            tbl = sql_table, pk = dialect.quote_ident(pk_n), max_params = max_params,
                        ).unwrap();
                    }
                }
            }
            ...
            if !table.config.is_view && !pk_cols.is_empty() && !update_cols.is_empty() {
                ...
                let bits_len = update_cols.len().div_ceil(8);
                write!(&mut code, 
                    ...
                    args = pk_args.join(", "), patch = patch_name, bits_len = bits_len
                ).unwrap();
                ...
                for (i, (n, _)) in update_cols.iter().enumerate() {
                    ...
                    write!(&mut code, 
                        ...
                        f = field, byte_idx = byte_idx, bit_idx = bit_idx
                    ).unwrap();
                }
                ...
                write!(&mut code, 
                    "        static CACHE: std::sync::OnceLock<[std::sync::RwLock<std::collections::HashMap<[u8; {bits_len}], String>>; 16]> = std::sync::OnceLock::new();\n\
                             let cache_shards = CACHE.get_or_init(|| std::array::from_fn(|_| std::sync::RwLock::new(std::collections::HashMap::new())));\n\
                             let shard_idx = bits.iter().fold(0usize, |acc, &b| acc.wrapping_add(b as usize) ^ (acc << 3)) % 16;\n\
                             ...
                             let query_str = {{\n\
                                 ...
                                 if let Some(q) = read.get(&bits) {{\n\
                                     ...
                                     if let Some(q) = write.get(&bits) {{\n\
                                         ...
                                         q.push_str(\"UPDATE {tbl} SET \");\n\
                                         let mut first = true;\n",
                    bits_len = bits_len, tbl = sql_table, cap = 256 + update_cols.len() * 32
                ).unwrap();

                if dialect == Dialect::Postgres {
                    code.push_str("                let mut param_idx = 1;\n");
                }
                ...
                for (n, _) in &update_cols {
                    ...
                    let ph = if dialect == Dialect::Postgres {
                        "{ use std::fmt::Write; write!(&mut q, \"${}\", param_idx).unwrap(); param_idx += 1; }"
                    } else {
                        "q.push_str(\"?\");"
                    };
                    write!(&mut code, 
                ...
                for (i, (n, _)) in pk_cols.iter().enumerate() {
                    ...
                    write!(&mut pk_where_gen, 
                        "                q.push_str(r#\"{conj}{cq} = \"#);\n\
                                         {ph}\n",
                        conj = conj, cq = col_q, ph = ph
                    ).unwrap();
                }
                code.push_str(&pk_where_gen);
                ...
                writeln!(&mut code, "        let mut query = sqlx::query::<{db}>(sqlx::AssertSqlSafe(query_str.as_str()));", db = db_type).unwrap();
                for (n, _) in &update_cols {
                    let field = Self::escape_rust_keyword(n);
                    writeln!(&mut code, 
            ...
            write_if_changed(&table_file, &code).map_err(|e| e.to_string())?;
        ...
        for (table_file, m1, m2) in results {
        ...
        write_if_changed(&mod_file, &mod_code)?;
