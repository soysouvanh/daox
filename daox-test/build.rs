...
async fn main() {
    ...
    generator_mysql.generate().await.expect("Failed to generate MySQL DAOs");
    ...
    generator_pg.generate().await.expect("Failed to generate Postgres DAOs");
    ...
    generator_sqlite.generate().await.expect("Failed to generate SQLite DAOs");
}