...
impl OrderItems {
    ...
    pub async fn get_by_pk<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(executor: E, order_id: &i64) -> sqlx::Result<Option<Self>> {
        let query = "SELECT * FROM order_items WHERE order_id = ?";
        sqlx::query_as::<_, Self>(query).bind(order_id).fetch_optional(executor).await
    }

    /// Vérifie si une ligne existe (Très léger, évite la RAM).
    pub async fn exists_by_pk<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(executor: E, order_id: &i64) -> sqlx::Result<bool> {
        let query = "SELECT 1 FROM order_items WHERE order_id = ? LIMIT 1";
        let exists: Option<(i32,)> = sqlx::query_as(query).bind(order_id).fetch_optional(executor).await?;
        Ok(exists.is_some())
    }
    ...
    pub async fn list_by_cursor<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(executor: E, last_id: &i64, limit: u32) -> sqlx::Result<Vec<Self>> {
        let query = "SELECT * FROM order_items WHERE order_id > ? ORDER BY order_id ASC LIMIT ?";
...
impl OrderItems {
    ...
    pub async fn update_by_pk<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(&self, executor: E) -> sqlx::Result<u64> {
        let query = "UPDATE order_items SET quantity = ? WHERE order_id = ?";
        let result = sqlx::query(query)
            .bind(&self.quantity)
            .bind(&self.order_id)
            .execute(executor).await?;
    ...
    pub async fn delete_by_pk<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(executor: E, order_id: &i64) -> sqlx::Result<u64> {
        let query = "DELETE FROM order_items WHERE order_id = ?";
        let result = sqlx::query(query).bind(order_id).execute(executor).await?;
        Ok(result.rows_affected())
    }
    ...
    pub async fn delete_many_by_pk<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(executor: E, ids: &[i64]) -> sqlx::Result<u64> {
        ...
        let mut query_builder: sqlx::QueryBuilder<sqlx::MySql> = sqlx::QueryBuilder::new("DELETE FROM order_items WHERE order_id IN ");
        query_builder.push("(");
...
impl OrderItems {
    ...
    pub async fn update_partial_by_pk<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(executor: E, order_id: &i64, patch: &OrderItemsPatch) -> sqlx::Result<u64> {
