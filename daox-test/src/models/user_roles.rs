...
impl UserRoles {
    ...
    pub async fn get_by_pk<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(executor: E, user_id: &i64) -> sqlx::Result<Option<Self>> {
        let query = "SELECT * FROM user_roles WHERE user_id = ?";
        sqlx::query_as::<_, Self>(query).bind(user_id).fetch_optional(executor).await
    }

    /// Vérifie si une ligne existe (Très léger, évite la RAM).
    pub async fn exists_by_pk<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(executor: E, user_id: &i64) -> sqlx::Result<bool> {
    ...
    pub async fn list_by_cursor<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(executor: E, last_id: &i64, limit: u32) -> sqlx::Result<Vec<Self>> {
        let query = "SELECT * FROM user_roles WHERE user_id > ? ORDER BY user_id ASC LIMIT ?";
...
impl UserRoles {
    ...
    pub async fn delete_by_pk<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(executor: E, user_id: &i64) -> sqlx::Result<u64> {
        let query = "DELETE FROM user_roles WHERE user_id = ?";
        let result = sqlx::query(query).bind(user_id).execute(executor).await?;
        Ok(result.rows_affected())
    }
    ...
    pub async fn delete_many_by_pk<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(executor: E, ids: &[i64]) -> sqlx::Result<u64> {
        ...
        let mut query_builder: sqlx::QueryBuilder<sqlx::MySql> = sqlx::QueryBuilder::new("DELETE FROM user_roles WHERE user_id IN ");
        query_builder.push("(");
    ...
    /// Met à jour la ligne via l'index `idx_user_id`.
    ...
    /// Supprime des lignes via l'index `idx_user_id`.
    ...
    /// Met à jour la ligne via l'index `idx_user_role`.
    ...
    /// Supprime des lignes via l'index `idx_user_role`.
...
impl UserRoles {
    ...
    pub async fn update_partial_by_pk<'e, E: sqlx::Executor<'e, Database = sqlx::MySql>>(executor: E, user_id: &i64, patch: &UserRolesPatch) -> sqlx::Result<u64> {
