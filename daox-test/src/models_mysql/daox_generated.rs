...
qb.push(" ON DUPLICATE KEY UPDATE `f_bool` = VALUES(`f_bool`), `f_decimal` = VALUES(`f_decimal`), `f_double` = VALUES(`f_double`), `f_float` = VALUES(`f_float`), `f_int` = VALUES(`f_int`), `f_text` = VALUES(`f_text`), `f_varchar` = VALUES(`f_varchar`)");
let result = qb.build().execute(&mut **executor).await?;
total_affected += result.rows_affected();
}
Ok(total_affected)
...
qb.push(" ON DUPLICATE KEY UPDATE `match` = VALUES(`match`), `type` = VALUES(`type`), `value` = VALUES(`value`)");
let result = qb.build().execute(&mut **executor).await?;
total_affected += result.rows_affected();
}
Ok(total_affected)
...
Ok(result.rows_affected())
    ...
    pub async fn delete_many_by_id<'e>(executor: &mut sqlx::Transaction<'e, sqlx::MySql>, ids: &[i32]) -> sqlx::Result<u64> {
if ids.is_empty() { return Ok(0); }
let mut total_affected = 0;
for chunk in ids.chunks(65535) {
...
Ok(result.rows_affected())
    ...
    pub async fn delete_many_by_code<'e>(executor: &mut sqlx::Transaction<'e, sqlx::MySql>, ids: &[String]) -> sqlx::Result<u64> {
if ids.is_empty() { return Ok(0); }
let mut total_affected = 0;
for chunk in ids.chunks(65535) {
...
qb.push(" ON DUPLICATE KEY UPDATE `quantity` = VALUES(`quantity`)");
let result = qb.build().execute(&mut **executor).await?;
total_affected += result.rows_affected();
}
Ok(total_affected)
...
Ok(result.rows_affected())
    ...
    pub async fn delete_many_by_id<'e>(executor: &mut sqlx::Transaction<'e, sqlx::MySql>, ids: &[Vec<u8>]) -> sqlx::Result<u64> {
if ids.is_empty() { return Ok(0); }
let mut total_affected = 0;
for chunk in ids.chunks(65535) {
